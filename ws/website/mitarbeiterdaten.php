...
if(mysqli_num_rows($result_birthday) > 0) {
    while ($row = mysqli_fetch_assoc($result_birthday)) {
        ...
        if ($row['GeburtstagFormatted'] == $today) {
            $birthday_message .= "<p style='$font_style;font-size: 28px;'>🎉 Herzlichen Glückwunsch ".$row['Vorname']." ".$row['Nachname']."! 🎉</p>";
        } else {
            ...
            $birthday_message .= "<p style='$font_style;font-size: 28px;'>🎉 Herzlichen Glückwunsch nachträglich zu Ihrem Geburtstag, ".$row['Vorname']." ".$row['Nachname']." vor ".$tage_seit_geburtstag." Tagen! 🎉</p>";
        }
    }
}
...
if(mysqli_num_rows($result_start_today) > 0) {
    while ($row = mysqli_fetch_assoc($result_start_today)) {
        ...
        if ($row['AnstelldatumFormatted'] == $today) {
            ...
            echo "<p style='$font_style;font-size: 28px;'>🎉 Herzlichen Glückwunsch zu Ihrem $jahre jährigen Betriebsjubiläum in unserem Unternehmen, ".$row['Vorname']." ".$row['Nachname']."! Danke für Ihre Arbeit seit dem: ".$anstelldatum->format('d.m.Y')."! 🎉</p>";
        } else {
            // Anstellungstag ist nicht heute
            $anstelldatum = new DateTime($row['Anstelldatum']);
            $heute = new DateTime('now');
            $jahre = $heute->diff($anstelldatum)->y;
            echo "<p style='$font_style;font-size: 28px;'>🎉 Herzlichen Glückwunsch nachträglich zu Ihrem $jahre jährigen Betriebsjubiläum ".$row['Vorname']." ".$row['Nachname'].". Danke für Ihre Arbeit seit dem: ".$anstelldatum->format('d.m.Y')."! 🎉</p>";
        }
    }
}
...
if (mysqli_num_rows($result_start_today) == 0 && empty($birthday_message)) {
    echo "<p style='$font_style;font-size: 28px;'>Heute hat niemand Geburtstag und es wurden auch keine Mitarbeiter eingestellt.</p>";
